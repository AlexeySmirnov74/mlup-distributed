...
def generate_openapi_schema(app: FastAPI, ml: MLupModel,model_dir_swagger):
    """
    openapi_schema["info"] = {
        "title": DOCS_TITLE,
    ...
    if "/health" in openapi_schema["paths"]:
        openapi_schema["paths"][model_dir_swagger+"/health"] = _fix_scheme_by_paths(
            openapi_schema["paths"][model_dir_swagger+"/health"], ['422', '429', '500'],
        #methods_to_remove = ["head", "options"]  # Удаляем методы HEAD и OPTIONS
        )
    if "/info" in openapi_schema["paths"]:
        openapi_schema["paths"][model_dir_swagger+"/info"] = _fix_scheme_by_paths(
            openapi_schema["paths"][model_dir_swagger+"/info"], ['422', '429', '500']
        )

    openapi_schema["paths"][model_dir_swagger+"/predict"] = {
      "post": {
        "summary": "Post json body in model. Return {predict_id}",
    ...
    if not ml.conf.get_first_model_argument:
...
def openapi_schema(app: FastAPI, ml: MLupModel,model_dir_swagger: str):

    if not app.openapi_schema:

        app.openapi_schema = generate_openapi_schema(app, ml,model_dir_swagger)
    return app.openapi_schema
