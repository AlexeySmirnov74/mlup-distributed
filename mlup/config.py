...
ENV_MODEL_REDIS = os.getenv("MODEL_REDIS", "redis")
ENV_MODEL_REDIS_PORT = os.getenv("MODEL_REDIS_PORT", "6379")

def get_redis_url():
    """
    Возвращает URL для подключения к Redis в зависимости от операционной системы
    и окружения.

    Параметры:
        whatport (int или str): порт для подключения к Redis.
    """
    whatredis=ENV_MODEL_REDIS
    whatport=ENV_MODEL_REDIS_PORT
    if platform.system() == "Windows":
        # Если запускается в Windows
        return f"redis://127.0.0.1:{whatport}"
    # Если понадобится условие для Docker, можно раскомментировать и использовать:
    # elif os.getenv("DOCKER_ENV", False):
    #     return f"redis://redis:{whatport}"
    else:
        # Значение по умолчанию для других систем
        return f"redis://{whatredis}:{whatport}"
