...
class ModelConfig:
    ...
    get_first_model_argument:bool = False
...
class MLupModel(ModelConfig):
    ...
    async def _predict(self, data_for_predict: Optional[Any] = None, **other_predict_args):
        ...
        with TimeProfiler('End model work. The model work in seconds:',log_level='debug'):
            try:
    ...
    def load(self, force_loading: bool = False):
        ...
        with TimeProfiler('Time to load binary model to memory:',log_level='debug'):
            try:
        ...
        logger.debug(f'Size deserializing models: {sys.getsizeof(self._model_obj, default=-1)}')
    ...
    def load_model_settings(self):
        ...
        logger.debug('Run load model settings.')
        if not self.loaded:
            raise ModelLoadError('Model not loaded to memory. Analyze impossible. Please call ml.load().')

        with TimeProfiler('Time to load model settings:',log_level='debug'):
